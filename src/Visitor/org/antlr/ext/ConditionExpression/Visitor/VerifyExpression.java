...
public class VerifyExpression extends BaseExpression {
	
	public VerifyExpression(Tree tree) {
		super(tree);
	}

	public VerifyExpression(BaseExpression exp, Tree tree) {
		super(exp, tree);
	}
	...
	public Object Evaluate(Object data, Object local) throws Exception {
		...
		case ExpressionLexer.DOT:
			leftResult = new VerifyExpression(_tree.getChild(0)).Evaluate(data, local);
			rightResult = new VerifyExpression(_tree.getChild(1)).Evaluate(data, local);
			return null;
		...
		case ExpressionLexer.FUNCTION:
			if (_tree.getParent() != null
				...
				for (int i = 0; i < _tree.getChildCount() - 1; i++) {
					params[i] = new VerifyExpression(_tree.getChild(i + 1)).Evaluate(data, local);
				...
				for (int i = 0; i < _tree.getChildCount() - 1; i++) {
					try {
						obj = new VerifyExpression(_tree.getChild(i + 1)).Evaluate(data, local);
					} catch (Exception e) {
